...
class ClientWorld extends PositionedObject {
  ...
  renderDynamicLayer(time, layerId, rangeCells) {
    ...
    const { startCell, endCell } = rc;

    for (let cellRow = startCell.row; cellRow <= endCell.row; cellRow++) {
      for (let cellCol = startCell.col; cellCol <= endCell.col; cellCol++) {
        world[cellRow][cellCol].render(time, layerId);
      }
    }
  }
